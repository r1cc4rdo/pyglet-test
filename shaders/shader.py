...
class Shader:
    ...
    def createShader(self, strings, shader_type):
        count = len(strings)
        ...
        if not temp:
            ...
            log_buffer = create_string_buffer(temp.value)
            # retrieve the log text
            glGetShaderInfoLog(shader, temp, None, log_buffer)
    ...
    def link(self):
        ...
        if not temp:
            ...
            log_buffer = create_string_buffer(temp.value)
            # retrieve the log text
            glGetProgramInfoLog(self.handle, temp, None, log_buffer)
            # print the log to the console
            print log_buffer.value
        else:
    ...
    def uniformf(self, name, *vals):
        ...
        if len(vals) in range(1, 5):  # retrieve the uniform location, and set
            {1: glUniform1f,
             2: glUniform2f,
             3: glUniform3f,
             4: glUniform4f}[len(vals)](glGetUniformLocation(self.handle, name), *vals)
    ...
    def uniformi(self, name, *vals):
        ...
        if len(vals) in range(1, 5):  # retrieve the uniform location, and set
            {1: glUniform1i,
             2: glUniform2i,
             3: glUniform3i,
             4: glUniform4i}[len(vals)](glGetUniformLocation(self.handle, name), *vals)
